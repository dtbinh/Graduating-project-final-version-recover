...
for i = 1:16
    ...
    Z = X1.*P{i}{1}(1,1).*X1 + X2.*P{i}{1}(2,2).*X2 + 2*X1.*P{i}{1}(1,2).*X2;
    %figure
...
for i = 1:length(Kalpha)
    Pa11 = Pa11 + h1.^Kalpha(i,2).*h2.^Kalpha(i,1).*P{i}{1}(1,1);
    Pa22 = Pa22 + h1.^Kalpha(i,2).*h2.^Kalpha(i,1).*P{i}{1}(2,2);
    Pa12 = Pa12 + h1.^Kalpha(i,2).*h2.^Kalpha(i,1).*P{i}{1}(1,2);
end
